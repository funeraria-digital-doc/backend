...
def get_upload_path(instance, filename):
    ...
    if instance.name is not None:
        if instance.group_id is not None and Group.objects.filter(id=instance.group_id).exists():
            group_name = Group.objects.filter(id=instance.group_id).first().name
            return "records/" + group_name + "/" + instance.name + '/' + filename
        print("records/default/" + instance.name + '/' + filename)
...
class Record(models.Model):
    ...
    marital_status = models.CharField(max_length=64, choices=MaritalStatus.choices,  db_column='marital_status', null=True, blank=True) 
    ...
    spouse_gender = models.CharField(max_length=64, choices=Gender.choices, db_column='spouse_gender',null=True, blank=True)
...
def delete_record_files(sender, instance=None, **kwargs):
    ...
    if os.path.exists(old_img):
        import shutil
        shutil.rmtree(os.path.abspath(os.path.dirname(old_img)),ignore_errors=True)
